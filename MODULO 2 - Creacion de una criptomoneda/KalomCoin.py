...
# Crear la direccion del nodo en el puerto 5000
...
def mine_block():
    ...
    blockchain.add_transaction(sender = node_address, receiver = "kalom", amount = 10)
...
def add_transaction():
    ...
    response = {
        'message': f'La transaccion sera añadida al bloque {index}'
    }
...
def connect_node():
    ...
    response = {
        'message' : 'Todos los nodos han sido conectados. La cadena de KalomCoin contiene ahora los nodos siguientes:',
        'total_dondes' : list(blockchain.nodes)
    }
    return jsonify(response), 201
...
def replace_chain():
    ...
    if is_chain_replace:
        response = {
            'message' : 'Los nodos tenian diferentes cadenas, que han sido todas reemplazadas por la mas larga',
